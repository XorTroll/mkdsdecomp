...
namespace {
    ...
    EmblemColor g_EditEmblemPixelData[EmblemWidthHeight * EmblemWidthHeight];
    constexpr auto EmblemGridColor = IM_COL32(120, 120, 120, 255);
    int g_GridSpacing = 1;
    EmblemColor g_PaintColor = EmblemColor_White;
    ...
    inline void SetEmblemPixel(const u32 x, const u32 y, EmblemColor clr) {
        g_EditEmblemPixelData[y * EmblemWidthHeight + x] = clr;
    }

    inline EmblemColor GetEmblemPixel(const u32 x, const u32 y) {
        return g_EditEmblemPixelData[y * EmblemWidthHeight + x];
    }

    inline bool HasSaveDataLoaded() {
        return g_SaveData.header.magic_nksy == MagicNKSY;
    }
        ...
        for(u32 cup = 0; cup < 8; cup++) {
            for(u32 cc = 0; cc < 4; cc++) {
                ...
                if(edit_info->valid) {
                    ...
                    edit_info->character_id = CharacterId_Mario;
                    edit_info->kart_id = KartId_StandardMR;
                    edit_info->points = 5;
                    edit_info->rank = Rank_E;
                    edit_info->cup_res = CupResult_Bronze;
        ...
        for(u32 c = 0; c < 32; c++) {
            for(u32 r = 0; r < 6; r++) {
                ...
                if(edit_record->valid) {
                    ...
                    edit_record->character_id = CharacterId_Mario;
                    edit_record->kart_id = KartId_StandardMR;
        ...
        for(u32 l = 0; l < 7; l++) {
            for(u32 s = 0; s < 9; s++) {
                ...
                if(g_MissionRunStageInfos[l][s].beaten) {
                    ...
                    g_MissionRunStageInfos[l][s].rank = Rank_E;
                }
            }
        }

        g_EditEmblemEnabled = g_SaveData.em.has_emblem != 0;
        for(u32 x = 0; x < EmblemWidthHeight; x++) {
            for(u32 y = 0; y < EmblemWidthHeight; y++) {
                const auto clr = GetEmblemColorAt(g_SaveData.em.emblem_data, x, y);
    ...
    void LoadSaveDataFields() {
        g_SaveData = {};
        if(ReadSaveData(g_SaveData, g_RawSaveData, g_RawSaveDataSize)) {
            ...
            if(ImGui::BeginMenu("File")) {
                ...
                if(HasSaveDataLoaded()) {
                    ...
                    if(ImGui::MenuItem("Save")) {
                        SaveEditFields();
                        WriteSaveData(g_SaveData, g_RawSaveData, g_RawSaveDataSize);
                    ...
                    if(ImGui::Button("Set all at max rank")) {
                        for(u32 l = 0; l < 7; l++) {
                            for(u32 s = 0; s < 9; s++) {
                                ...
                                g_MissionRunStageInfos[l][s].rank = Rank_Star3;
                            }
                        }
                    }
                    ImGui::SameLine();
                ...
                if(ImGui::CollapsingHeader("Edit emblem")) {
                    ...
                    for(u32 i = 1; i < 16; i++) {
                        ...
                        if(ImGui::Button(button_label.c_str(), ImVec2(32, 32))) {
                            g_PaintColor = static_cast<EmblemColor>(i);
                        }
                    ...
                    for(u32 i = 0; i <= EmblemWidthHeight; i++) {
                        const auto v_pos1 = ImVec2(canvas_pos.x + i * g_GridSpacing + i * g_EmblemScale, canvas_pos.y);
                        const auto v_pos2 = ImVec2(v_pos1.x + g_GridSpacing, v_pos1.y + (EmblemWidthHeight + 1) * g_GridSpacing + EmblemWidthHeight * g_EmblemScale);
                        ...
                        const auto h_pos2 = ImVec2(h_pos1.x + (EmblemWidthHeight + 1) * g_GridSpacing + EmblemWidthHeight * g_EmblemScale, h_pos1.y + g_GridSpacing);
                    ...
                    if(is_in_canvas && (click_l || click_r)) {
                        ...
                        if((x < EmblemWidthHeight) && (y < EmblemWidthHeight)) {
                            const auto clr = click_r ? EmblemColor_Transparent : g_PaintColor;
                            SetEmblemPixel(x, y, clr);
                        }
                    }

                    ImGui::SameLine();
                    const auto space_wh = EmblemWidthHeight * g_EmblemScale + (EmblemWidthHeight + 1) * g_GridSpacing + 5 * (g_EmblemScale + g_GridSpacing);
