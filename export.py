...
def export_all_to_header(out_file):
    with open(out_file, 'w') as f:
        now = datetime.now()

        f.write("/* MKDS (EU) ARM9 struct, enum and function definitions */\n")
        f.write(f"/* Generated by mkdsdecomp: https://github.com/XorTroll/mkdsdecomp, at {now.strftime('%B %d, %Y %I:%M:%S %p')} */\n\n")

        f.write("#ifndef MKDSDECOMP_GEN_H\n")
        f.write("#define MKDSDECOMP_GEN_H\n\n")
        ...
        f.write("#endif // MKDSDECOMP_GEN_H\n")

    print(f"Done, exported header to '{out_file}'")
...
header_path = ida_kernwin.ask_file(1, "*.h", "Export C header file")
if header_path is not None:
    export_all_to_header(header_path)
