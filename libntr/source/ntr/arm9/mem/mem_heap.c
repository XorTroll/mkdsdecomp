...
#include <ntr/arm9/util.h>
#include <ntr/arm9/os/os_irq.h>

#include <ntr/extra/extra_log.h>
#define _NTR_ARM9_MEM_DEBUGLOGF(fmt, ...) NTR_EXTRA_DEBUGLOGF("[mem] " fmt, __VA_ARGS__)
...
#define _NTR_ARM9_MEM_REGION_SIZE(region) (size_t)((region)->end - (region)->start)

#define _NTR_ARM9_MEM_EXP_HEAP_HEAD_MAGIC 0x45585048 // "EXPH"

#define _NTR_ARM9_MEM_EXP_HEAP_FREE_BLOCK_HEAD_MAGIC 0x4652 // "FR"
#define _NTR_ARM9_MEM_EXP_HEAP_USED_BLOCK_HEAD_MAGIC 0x5544 // "UD"
...
#define _NTR_ARM9_MEM_EXP_GET_BLOCK(block_head) (((uintptr_t)block_head) + sizeof(Mem_ExpHeapMemoryBlockHead))
#define _NTR_ARM9_MEM_EXP_GET_PTR_BLOCK_HEAD(ptr) ((Mem_ExpHeapMemoryBlockHead*)(((uintptr_t)ptr) - sizeof(Mem_ExpHeapMemoryBlockHead)))

#define _NTR_ARM9_MEM_EXP_GET_MEMORY_BLOCK_ALIGNMENT_PADDING(block_head) (u16)(((block_head)->attributes >> 8) & 0x7F)
...
#define _NTR_ARM9_MEM_FRAME_HEAP_HEAD_MAGIC 0x46524D48 // "FRMH"
...
void Mem_GetMemoryBlockRegion(Mem_MemoryRegion *out_region, Mem_ExpHeapMemoryBlockHead *block_head) {
    out_region->start = (uintptr_t)block_head - (uintptr_t)(_NTR_ARM9_MEM_EXP_GET_MEMORY_BLOCK_ALIGNMENT_PADDING(block_head));
    out_region->end = _NTR_ARM9_MEM_EXP_GET_BLOCK(block_head) + block_head->block_size;
}
...
size_t Mem_Frame_GetAllocatableSize(Mem_HeapHead *heap_head, ssize_t align) {
    ...
    uintptr_t cur_heap_start_aligned = NTR_ARM9_UTIL_ALIGN_UP(heap_head->frm_heap_head.cur_heap_region.start, actual_align);
...
void *Mem_Frame_AllocFromHead(Mem_FrameHeapHead *frm_heap_head, size_t size, size_t align) {
    uintptr_t cur_ptr_start_aligned = NTR_ARM9_UTIL_ALIGN_UP(frm_heap_head->cur_heap_region.start, align);
...
void *Mem_Frame_AllocFromTail(Mem_FrameHeapHead *frm_heap_head, size_t size, size_t align) {
    uintptr_t cur_heap_end_aligned = NTR_ARM9_UTIL_ALIGN_DOWN(frm_heap_head->cur_heap_region.end - size, align);
...
void *Mem_Frame_Allocate(Mem_HeapHead *heap_head, size_t size, ssize_t align) {
    ...
    size_t aligned_size = NTR_ARM9_UTIL_ALIGN_UP(size, 0x4);
...
Mem_HeapHandle Mem_Frame_Initialize(Mem_HeapHead *heap_head, uintptr_t end, int option) {
    Mem_InitializeHeapHead(heap_head, _NTR_ARM9_MEM_FRAME_HEAP_HEAD_MAGIC, (uintptr_t)heap_head + sizeof(Mem_HeapHead), end, option);
...
Mem_HeapHandle Mem_Frame_Create(void *ptr, size_t size, int option) {
    uintptr_t aligned_start = NTR_ARM9_UTIL_ALIGN_UP((uintptr_t)ptr, 0x4);
    uintptr_t aligned_end = NTR_ARM9_UTIL_ALIGN_DOWN((uintptr_t)(ptr + size), 0x4);
...
Mem_HeapHandle Mem_Exp_Initialize(uintptr_t start, uintptr_t end, u8 option) {
    ...
    Mem_InitializeHeapHead(heap_head, _NTR_ARM9_MEM_EXP_HEAP_HEAD_MAGIC, start + sizeof(Mem_HeapHead), end, option);
    exp_heap_head->group_id = 0;
    exp_heap_head->alloc_mode &= ~0x1u;
    exp_heap_head->alloc_mode = Mem_AllocationMode_FirstFit;

    Mem_ExpHeapMemoryBlockHead *start_free_block = Mem_Exp_InitializeMemoryBlock(&heap_head->heap_region, _NTR_ARM9_MEM_EXP_HEAP_FREE_BLOCK_HEAD_MAGIC);
...
Mem_HeapHandle Mem_Exp_Create(void *ptr, size_t size, u8 option) {
    uintptr_t end = NTR_ARM9_UTIL_ALIGN_DOWN((uintptr_t)(ptr + size), 0x4);
    uintptr_t start = NTR_ARM9_UTIL_ALIGN_UP((uintptr_t)ptr, 0x4);

    if((start <= end) && ((end - start) >= 0x4C)) { // TODO: where does this size come from?
        _NTR_ARM9_MEM_DEBUGLOGF("Mem_Exp_Create: start=%p, end=%p", (void*)start, (void*)end);
...
size_t Mem_Exp_GetAllocatableSize(Mem_HeapHead *heap_head, ssize_t align) {
    ...
    if(cur_block_head != NULL) {
        do {
            uintptr_t cur_block_start_aligned = NTR_ARM9_UTIL_ALIGN_UP(_NTR_ARM9_MEM_EXP_GET_BLOCK(cur_block_head), actual_align);
            uintptr_t cur_block_end = _NTR_ARM9_MEM_EXP_GET_BLOCK(cur_block_head) + cur_block_head->block_size;
            if((cur_block_start_aligned < cur_block_end) && ((max_allocatable_size < (cur_block_end - cur_block_start_aligned)) || (max_allocatable_size == (cur_block_end - cur_block_start_aligned))) && (min_offset > (cur_block_start_aligned - _NTR_ARM9_MEM_EXP_GET_BLOCK(cur_block_head)))) {
                max_allocatable_size = cur_block_end - cur_block_start_aligned;
                min_offset = cur_block_start_aligned - _NTR_ARM9_MEM_EXP_GET_BLOCK(cur_block_head);
            }
...
void *Mem_Exp_ConvertFreeBlockToUsedBlock(Mem_ExpHeapHead *exp_heap_head, Mem_ExpHeapMemoryBlockHead *block_head, uintptr_t block, size_t size, Mem_AllocationDirection mem_block_alloc_direction) {
    ...
    free_region_front.end = (uintptr_t)_NTR_ARM9_MEM_EXP_GET_PTR_BLOCK_HEAD(block);
    ...
    if(_NTR_ARM9_MEM_REGION_SIZE(&free_region_front) >= sizeof(Mem_ExpHeapMemoryBlockHead)) {
        Mem_ExpHeapMemoryBlockHead *new_free_block = Mem_Exp_InitializeMemoryBlock(&free_region_front, _NTR_ARM9_MEM_EXP_HEAP_FREE_BLOCK_HEAD_MAGIC);
    ...
    if(_NTR_ARM9_MEM_REGION_SIZE(&free_region_back) >= sizeof(Mem_ExpHeapMemoryBlockHead)) {
        Mem_ExpHeapMemoryBlockHead *new_used_block = Mem_Exp_InitializeMemoryBlock(&free_region_front, _NTR_ARM9_MEM_EXP_HEAP_USED_BLOCK_HEAD_MAGIC);
        Mem_ExpHeapMemoryBlockList_Insert(&exp_heap_head->free_list, new_used_block, prev_block_head);
    }
    else {
        free_region_back.start = free_region_back.end;
    }

    if(_NTR_EXP_GET_BASE_HEAP_HEAD(exp_heap_head)->option & Mem_CreateOption_ZeroClear) {
        _NTR_ARM9_MEM_DEBUGLOGF("Mem_Exp_ConvertFreeBlockToUsedBlock: fill32 ptr=%p size=%d", (void*)free_region_front.end, free_region_back.start - free_region_front.end);
        Mem_CpuFill32(0, (void*)free_region_front.end, free_region_back.start - free_region_front.end);
    }

    Mem_MemoryRegion used_region;
    used_region.start = (uintptr_t)_NTR_ARM9_MEM_EXP_GET_PTR_BLOCK_HEAD(block);
    used_region.end = free_region_back.start;
    Mem_ExpHeapMemoryBlockHead *used_block = Mem_Exp_InitializeMemoryBlock(&used_region, _NTR_ARM9_MEM_EXP_HEAP_USED_BLOCK_HEAD_MAGIC);
    
    // Set allocation direction
    used_block->attributes &= ~0x8000u;
    used_block->attributes |= (mem_block_alloc_direction & 1) << 15;

    // Set memory block alignment padding
    u8 mem_block_align_pad = (((uintptr_t)used_block & 0xFF) - (free_region_front.end & 0xFF));
    used_block->attributes &= ~0x7F00u;
    used_block->attributes |= (mem_block_align_pad & 0x7F) << 8;

    // Set group ID
    used_block->attributes &= ~0xFFu;
    used_block->attributes |= exp_heap_head->group_id & 0xFF;
...
int Mem_Exp_CoalesceFreedRegion(Mem_ExpHeapHead *heap_head, Mem_MemoryRegion *region) {
    ...
    if(cur_free_block != NULL) {
        ...
        if((uintptr_t)cur_free_block == region->end) {
            free_region.end = _NTR_ARM9_MEM_EXP_GET_BLOCK(cur_free_block) + cur_free_block->block_size;
            Mem_ExpHeapMemoryBlockList_Remove(&heap_head->free_list, cur_free_block);
        }
    }

    if((prev_free_block != NULL) && ((_NTR_ARM9_MEM_EXP_GET_BLOCK(prev_free_block) + prev_free_block->block_size) == region->start)) {
    ...
    if(_NTR_ARM9_MEM_REGION_SIZE(&free_region) >= sizeof(Mem_ExpHeapMemoryBlockHead)) {
        Mem_ExpHeapMemoryBlockHead *new_free_block = Mem_Exp_InitializeMemoryBlock(&free_region, _NTR_ARM9_MEM_EXP_HEAP_FREE_BLOCK_HEAD_MAGIC);
...
void *Mem_Exp_AllocateFromHead(Mem_HeapHead *heap_head, size_t size, size_t align) {
    _NTR_ARM9_MEM_DEBUGLOGF("Mem_Exp_AllocateFromHead: size=%d, align=%d", size, align);
    ...
    if(cur_free_block_head != NULL) {
        do {
            ...
            uintptr_t cur_block = _NTR_ARM9_MEM_EXP_GET_BLOCK(cur_free_block_head);
            uintptr_t block_offset = NTR_ARM9_UTIL_ALIGN_UP(_NTR_ARM9_MEM_EXP_GET_BLOCK(cur_free_block_head), align);
            ...
            if((cur_block_size >= (size + block_offset - cur_block)) && (best_size > cur_block_size)) {
                ...
                if(is_first_fit || (cur_block_size == size)) {
                    break;
                }
            }
...
void *Mem_Exp_AllocateFromTail(Mem_HeapHead *heap_head, size_t size, size_t align) {
    _NTR_ARM9_MEM_DEBUGLOGF("Mem_Exp_AllocateFromTail: size=%d, align=%d", size, align);
    ...
    if(cur_free_block_head != NULL) {
        do {
            size_t cur_block_size = cur_free_block_head->block_size;
            uintptr_t cur_block = _NTR_ARM9_MEM_EXP_GET_BLOCK(cur_free_block_head);
            uintptr_t block_offset = NTR_ARM9_UTIL_ALIGN_DOWN(_NTR_ARM9_MEM_EXP_GET_BLOCK(cur_free_block_head) + cur_block_size - size, align);
            // Check if the data would fit in this block
...
void *Mem_Exp_Allocate(Mem_HeapHead *heap_head, size_t size, ssize_t align) {
    _NTR_ARM9_MEM_DEBUGLOGF("Mem_Exp_Allocate: size=%d, align=%d", size, align);
...
void Mem_Exp_Free(Mem_HeapHead *heap_head, void *ptr) {
    ...
    Mem_ExpHeapMemoryBlockHead *block_head = _NTR_ARM9_MEM_EXP_GET_PTR_BLOCK_HEAD(ptr);
...
Mem_HeapHandle Mem_CreateExpHeap(void *ptr, size_t size) {
    _NTR_ARM9_MEM_DEBUGLOGF("Mem_CreateExpHeap: ptr=%p, size=0x%X", ptr, size);
...
void Mem_DestroyHeap(Mem_HeapHandle heap_handle) {
    ...
    if(heap_handle->magic == _NTR_ARM9_MEM_EXP_HEAP_HEAD_MAGIC) {
        return Mem_Exp_Finalize(heap_handle);
    }
    else if(heap_handle->magic == _NTR_ARM9_MEM_FRAME_HEAP_HEAD_MAGIC) {
    ...
    if(parent_heap_head != NULL) {
        ...
        if(parent_heap_head->magic == _NTR_ARM9_MEM_EXP_HEAP_HEAD_MAGIC) {
...
void *Mem_AllocateHeap(Mem_HeapHandle heap_handle, size_t size) {
    ...
    if(heap_handle->magic == _NTR_ARM9_MEM_FRAME_HEAP_HEAD_MAGIC) {
...
void Mem_ResizeExpHeap(Mem_HeapHandle heap_handle, void *ptr, size_t size) {
    Mem_ExpHeapMemoryBlockHead *ptr_block = _NTR_ARM9_MEM_EXP_GET_PTR_BLOCK_HEAD(ptr);
    size_t aligned_size = NTR_ARM9_UTIL_ALIGN_UP(size, 0x4);
    ...
    if(aligned_size != orig_block_size) {
        if(aligned_size <= orig_block_size) {
            ...
            if((i != NULL) && (aligned_size <= (orig_block_size + i->block_size + sizeof(Mem_ExpHeapMemoryBlockHead)))) {
                ...
                region.start = (uintptr_t)ptr + aligned_size;
                if(_NTR_ARM9_MEM_REGION_SIZE(&region) < sizeof(Mem_ExpHeapMemoryBlockHead)) {
                    region.end = region.start;
                }
                ptr_block->block_size = aligned_size;
                if(_NTR_ARM9_MEM_REGION_SIZE(&region) >= sizeof(Mem_ExpHeapMemoryBlockHead)) {
                    Mem_ExpHeapMemoryBlockHead *new_free_block = Mem_Exp_InitializeMemoryBlock(&region, _NTR_ARM9_MEM_EXP_HEAP_FREE_BLOCK_HEAD_MAGIC);
...
Mem_HeapHandle Mem_CreateChildExpHeap(Mem_HeapHandle parent_heap_handle) {
    ...
    if(parent_heap_handle->magic == _NTR_ARM9_MEM_EXP_HEAP_HEAD_MAGIC) {
        allocatable_size = Mem_Exp_GetAllocatableSize(parent_heap_handle, 0x4);
        _NTR_ARM9_MEM_DEBUGLOGF("Mem_CreateChildExpHeap: parent ExpHeap allocatable_size=%d", allocatable_size);
    }
    else if(parent_heap_handle->magic == _NTR_ARM9_MEM_FRAME_HEAP_HEAD_MAGIC) {
    ...
    if(parent_heap_handle->magic == _NTR_ARM9_MEM_EXP_HEAP_HEAD_MAGIC) {
        ...
        _NTR_ARM9_MEM_DEBUGLOGF("Mem_CreateChildExpHeap: parent ExpHeap alloc ptr=%p", allocated_ptr);
    }
    else if(parent_heap_handle->magic == _NTR_ARM9_MEM_FRAME_HEAP_HEAD_MAGIC) {
